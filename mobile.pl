...
<% main_page_thread_list() %>
  ...
  <form id="threadform" class="postform" method="post" action="<%= $ENV{SCRIPT_NAME} %>">
   <p>
...
%>
  ...
  <p>*: <a href="<%= $ENV{SCRIPT_NAME} %>/subback/"><%= S_ALLTHR %></a></p>
...
<%
 ...
 sub show_post($$$$$$$$$$$){
  ...
  $file=join '.',@fileparts if @fileparts;
  if($deleted){
...
%>
    ...
    <p class="threadclosed" id="replyform"><%= S_CLOSED %></p>

<% }else{ %>

    <form class="postform" method="post" action="<%= $ENV{SCRIPT_NAME} %>" id="replyform">
     <p>
      <input type="hidden" name="task" value="post" />
      <input type="hidden" name="thread" value="<%= $thread %>" />
      <span style="display:none">
       <label for="name">Spam: </label><br />
       <input type="text" name="name" id="name" />
       <label for="link">Spam: </label>
       <input type="text" name="link" id="link" /><br />
      </span>
      <label for="field1-<%= $thread %>"><%= S_NAME %></label>
      <input type="text" name="field1" id="field1-<%= $thread %>" /><br />
      <label for="field2-<%= $thread %>"><%= S_LINK %></label>
      <input type="text" name="field2" id="field2-<%= $thread %>" /><br />
      <label for="comment-<%= $thread %>"><%= S_COMMENT %></label>
      <textarea name="comment" id="comment-<%= $thread %>" rows="6" cols="60"></textarea><br />
      <label for="file-<%= $thread %>"><%= S_IMAGE %></label>
      <input type="file" name="file" id="file-<%= $thread %>" />
      <input type="submit" value="<%= S_REPLY %>" />
     </p>
    </form>
